...
### Router Migration (Session 18–19)
Full migration to `next/navigation` for App Router compatibility — all components now use `next/navigation` exclusively:
- **`next/navigation`**: `AdminSidebar`, `Breadcrumb`, `AdminHeader`, `useListView`, all page components (`customer-service`, `customer-service/[id]`, `users/index`, `users/[id]`, `setup/index`)
